...
public class MonthSelection extends BaseSelection<YearMonth> {
    ...
    protected boolean isNextButtonCheck() {
        if(isBlockedByTime)return getValue().plusMonths(1).isBefore(time.getYearMonth().plusYears(2));
        return true;
    }
    @Override
    protected boolean isPreviousButtonCheck() {
        if(isBlockedByTime)return (getValue().minusMonths(1).isAfter(time.getYearMonth().minusYears(2)) && getValue().isAfter(YearMonth.from(time.startingTime)));
        return true;
    }
    @Override
