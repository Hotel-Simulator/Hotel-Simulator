...
public class CalendarComponent extends WrapperTable {
	...
	private final MonthSelection monthSelection = new MonthSelection(YearMonth.from(time.getTime()),this::monthSelectionHandler);
	private final YearSelection yearSelection = new YearSelection(YearMonth.from(time.getTime()),this::yearSelectionHandle);

	public CalendarComponent(LocalDate chosenDate, Consumer<LocalDate> dateChangeHandler) {
	...
	private class CalendarCellButton extends WrapperContainer<TextButton> {
		...
		public void setStyle(TextButton button,LocalDate date){
			if(eventsMap.containsKey(date)) button.setStyle(skin.get("calendar-special-cell", TextButton.TextButtonStyle.class));
			else button.setStyle(skin.get("calendar-cell", TextButton.TextButtonStyle.class));
		}
		...
		public void addEventListener(TextButton button,LocalDate date){
			...
			if(eventsMap.containsKey(date)){
				button.addListener(new LanguageToolTip(eventsMap.get(date).description()));
			}
		}
