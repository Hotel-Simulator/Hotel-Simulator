...
import static pl.agh.edu.engine.bank.TransactionType.RECEIVED_CREDIT;
...
public class BankAccount {
	...
	private void chargeAccountFee() {
		registerExpense(ACCOUNT_FEE_CHARGE, accountDetails.accountFee());
	}

	public void registerIncome(TransactionType transactionType, BigDecimal value) {
		transactions.add(new Transaction(transactionType, value, time.getTime()));
		balance = balance.add(value);
	}

	public void registerExpense(TransactionType transactionType, BigDecimal value) {
		transactions.add(new Transaction(transactionType, value, time.getTime()));
	...
	public void registerCredit(Credit credit) {
		credits.add(credit);
		registerIncome(RECEIVED_CREDIT, credit.value);
	}
	...
	public List<Transaction> getIncomeList() {
		return transactions.stream()
				.filter(transaction -> transaction.type().isIncome)
				.toList();
	}
