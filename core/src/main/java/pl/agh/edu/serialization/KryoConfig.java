...
public class KryoConfig {
	...
	private void registerOpinion(Kryo kryo) {
	...
	private void registerClientGroup(Kryo kryo) {
	...
	private void registerPossibleEmployee(Kryo kryo) {
	...
	private void registerEmployee(Kryo kryo) {
	...
	private void registerEvents(Kryo kryo) {
		...
		kryo.register(ClientNumberModifier.class, new Serializer<ClientNumberModifier>() {
			@Override
		...
		kryo.register(ClientNumberModificationTemporaryEvent.class, new Serializer<ClientNumberModificationTemporaryEvent>() {
			@Override
	...
	private void registerHotel(Kryo kryo) {
		kryo.register(DifficultyLevel.class);
		kryo.register(HotelType.class);
	}

	private void registerRoom(Kryo kryo) {
		kryo.register(RoomRank.class);
		kryo.register(RoomSize.class);
		kryo.register(RoomState.class, new Serializer<RoomState>() {
			@Override
			...
			public RoomState read(Kryo kryo, Input input, Class<? extends RoomState> type) {
				...
				roomState.setOccupied(kryo.readObject(input, Boolean.class));
				roomState.setDirty(kryo.readObject(input, Boolean.class));
				roomState.setFaulty(kryo.readObject(input, Boolean.class));
				roomState.setUnderRankChange(kryo.readObject(input, Boolean.class));
				roomState.setBeingBuild(kryo.readObject(input, Boolean.class));
		...
		kryo.register(Room.class, new Serializer<Room>() {
			...
			public void write(Kryo kryo, Output output, Room object) {
				kryo.writeObject(output, object.getRank());
				kryo.writeObject(output, object.size);
				kryo.writeObject(output, object.roomState);
				kryo.writeObjectOrNull(output, object.getResidents(), ClientGroup.class);
			}
			...
			public Room read(Kryo kryo, Input input, Class<? extends Room> type) {
				...
				RoomState state = kryo.readObject(input, RoomState.class);
				room.roomState.setOccupied(state.isOccupied());
				room.roomState.setDirty(state.isDirty());
				room.roomState.setFaulty(state.isFaulty());
				room.roomState.setUnderRankChange(state.isUnderRankChange());
				room.roomState.setBeingBuild(state.isBeingBuild());

				setPrivateFieldValue(room, "residents", kryo.readObjectOrNull(input, ClientGroup.class));
	...
	// private <T extends Enum<T>, K> Serializer<EnumMap<T,K>> enumMapSerializer(Class<T> tClass, Class<K> kClass) {
	// return new Serializer<EnumMap<T, K>>() {
	// @Override
	// public void write(Kryo kryo, Output output, EnumMap<T, K> object) {
	//
	// }
	//
	// @Override
	// public EnumMap<T, K> read(Kryo kryo, Input input, Class<? extends EnumMap<T, K>> type) {
	// return null;
	// }
	// };
	// }
